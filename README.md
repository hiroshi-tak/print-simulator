...
GoogleTestを使用して、各クラスおよび`Printer`の動作を単体テストしています。
...
| テスト対象             |   テスト数 | 内容                      |
| ----------------- | -----: | ----------------------- |
| Motor             |      3 | 初期状態、モーター開始、停止          |
| Sensor            |      6 | 用紙検出、用紙なし、紙詰まり検出・解除     |
| Timer             |      4 | タイマー開始、カウント、停止、再開始      |
| PrintJob          |      6 | ジョブ作成、印刷、残部数管理、異常値      |
| Printer           |     12 | 印刷開始・停止、エラー、ジョブ実行、インク切れ |
| MockPrinterDevice |      3 | ハードウェア依存部をモック化した印刷処理    |
| **合計**            | **34** | **34テストすべて成功**          |
...
* 初期状態では印刷ジョブが存在しないこと
* 印刷ジョブを作成できること
* 1部ずつ印刷すると残り部数が減少すること
* すべて印刷するとジョブが完了すること
* 残部数が0の場合は印刷できないこと
* 0部以下の不正なジョブを登録できないこと
...
* 印刷を停止できること
* 印刷を完了できること
* 紙詰まりを検出すると`ERROR`状態になること
* エラーを解除して復旧できること
* 印刷ジョブを登録できること
* 印刷を実行すると残り部数とインク残量が減少すること
* `IDLE`状態では印刷ジョブを実行できないこと
* インクがなくなると`INK_EMPTY`エラーになること

### モックを使用したテスト

`Printer`は`PrinterDevice`をポインタで受け取る設計とし、実際の`InkjetPrinter`や`LaserPrinter`に依存せずテストできるようにしています。

テストでは`MockPrinterDevice`を使用し、以下を確認しています。

* モックデバイスを使用して印刷できること
* 複数部の印刷を正しく処理できること
* デバイスの残量がなくなった場合に`INK_EMPTY`エラーになること

これにより、実際のプリンターデバイスを使用せず、`Printer`の制御ロジックを独立して検証しています。
...
単体テストおよび`MockPrinterDevice`を使用したテストがすべて成功しています。
